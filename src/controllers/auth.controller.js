...
const googleAuth = catchAsync(async (req, res) => {
  ...
  try {
    ...
    console.log('[google-auth backend] Token verified OK for email:', decoded.email);
  } catch (verifyErr) {
    console.error('[google-auth backend] Token verification FAILED:', verifyErr.message);
    throw ApiError.unauthorized('Invalid or expired Google sign-in — please try again');
  }

  const { email, name, picture, uid } = decoded;
  if (!email) throw ApiError.badRequest('Your Google account has no email on file');
